...
def estimate_weight_from_boxes(boxes_text):
    """Estimate weight from box count"""
    match = re.search(r'(\d+)\s*(?:box|boxes|carton|cartons)', boxes_text, re.IGNORECASE)
    if match:
        count = int(match.group(1))
        # Average electronics box weight ~10-15kg
        estimated_min = count * 8
        estimated_max = count * 15
...
def chat():
    ...
    if session_id not in session_memory:
        session_memory[session_id] = {'cargo_type': None, 'destination': None, 'weight': None, 'origin': None}
    
    ctx = session_memory[session_id]
    msg_lower = user_msg.lower()
    
    # FIX 1: Handle box count -> weight estimation
    if 'box' in msg_lower or 'boxes' in msg_lower or 'carton' in msg_lower:
        estimated_weight = estimate_weight_from_boxes(user_msg)
        if estimated_weight and not ctx.get('weight'):
            ctx['weight'] = estimated_weight
            reply = f"No problem 👍 we can estimate. For {user_msg} of electronics, weight is usually around {estimated_weight} kg.\n\nI'll help you refine this. What's the destination?"
            session_memory[session_id] = ctx
            return jsonify({'reply': reply})
    
    # FIX 2: Handle destination change
    if 'change destination' in msg_lower or 'change to' in msg_lower:
        dest_match = re.search(r'(?:change to|to)\s+([A-Z][a-z]+|[A-Z]+)', user_msg)
        if dest_match:
            new_dest = dest_match.group(1).title()
            ctx['destination'] = new_dest
            session_memory[session_id] = ctx
            reply = f"Got it 👍 changing destination to {new_dest}. Same shipment details apply.\n\nDo you want fastest or most cost-effective option?"
            return jsonify({'reply': reply})
    
    # FIX 3: Urgent/next-day delivery - don't overpromise
    if ('pickup today' in msg_lower or 'delivery tomorrow' in msg_lower or 'next day' in msg_lower) and 'international' in msg_lower:
        return jsonify({'reply': "For international shipments 👍 next-day delivery is usually not possible.\n\nBut we can check the fastest express option (1-3 days) depending on route. What's your destination?"})
    
    # FIX 4: Ready to book / Take this forward - STRONG CLOSING
    if any(word in msg_lower for word in ['take this forward', 'ready to book', 'let\'s proceed', 'finalize', 'close the deal']):
        return jsonify({'reply': "Perfect 👍 I'll take this forward.\n\nJust share your name and WhatsApp number, and our team will handle everything from pickup to delivery.\n\nWhat's the best number to reach you?"})
    
    # FIX 5: Handle "None" bug - clean up display
    weight_display = ctx.get('weight', '')
    if weight_display and weight_display.lower() == 'none':
        ctx['weight'] = None
        weight_display = ''
    
    dest_display = ctx.get('destination', '')
    if dest_display and dest_display.lower() == 'none':
    ...
    for key, value in cargo_map.items():
        if key in msg_lower and not ctx.get('cargo_type'):
    ...
    # Build clean response without "None"
    if weight == 'your' and dest == 'destination':
        product_line = f"{weight} {cargo}"
    elif weight == 'your':
        product_line = f"{weight} {cargo} to {dest}"
    elif dest == 'destination':
        product_line = f"{weight}kg {cargo}"
    else:
        product_line = f"{weight}kg {cargo} to {dest}"
    
    # Detect intent
    if any(word in msg_lower for word in ['cheapest', 'budget', 'save money']):
        reply = f"For {product_line}, sea freight saves 60-70% compared to air.\n\nSea: 20-30 days | Air: 3-7 days\n\nWant me to check current rates for you?"
    elif any(word in msg_lower for word in ['urgent', 'fast', 'quick', 'express']):
        reply = f"For urgent delivery to {dest}:\n\n✓ Express air: 3-5 days\n✓ Priority handling\n✓ Real-time tracking\n\nShall I check flight availability?"
    elif any(word in msg_lower for word in ['bulk', 'monthly', 'weekly']):
        reply = f"For regular shipments, we offer:\n\n✓ Volume discounts\n✓ Dedicated account manager\n✓ Priority space allocation\n\nWhat's your estimated monthly volume?"
    elif any(word in msg_lower for word in ['customs', 'clearance']):
        reply = f"Customs clearance is our expertise 👍\n\nWe handle document preparation, HS code classification, duty calculation, and compliance verification.\n\nNeed help with a specific shipment?"
    elif 'electronics' in msg_lower:
        reply = f"Electronics need extra care 👍\n\nFor {product_line}, air freight is recommended for safety (3-7 days).\n\nShall I check the air freight option for you?"
    elif weight != 'your' and dest != 'destination':
        reply = f"For {product_line}:\n\n✈️ Air: 3-7 days (faster, higher cost)\n🚢 Sea: 20-30 days (slower, saves 60-70%)\n\nWhich matters more - speed or cost?"
    else:
        system_prompt = """You are Aisea, a logistics consultant for PAS Freight.

Rules:
- Keep responses short and conversational
- Never show 'None' - use 'your' instead
- Don't overpromise delivery times
...
Be helpful and conversational."""
        ...
        try:
            ...
            reply = "Tell me what you'd like to ship - I'll help you figure out the best way."
    
    return jsonify({'reply': reply})

if __name__ == '__main__':
    print("=" * 60)
    print("Aisea - PERFECT 10/10 Chatbot")
    print("Open: http://localhost:5001")
    print("Fixed: No 'None' bug | Box weight estimation | Realistic urgency | Strong closing")
    print("=" * 60)
