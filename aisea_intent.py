    ...
    <div id="pasfreight-chat-window">
        ...
        <div class="chat-messages" id="chatMessages">
            <div class="message bot-message">
                <div class="bubble">Hi, I'm Aisea. Need help with shipping? Ask me for quotes, strategy advice, or business scaling.</div>
            </div>
            <div class="message bot-message">
                <div class="bubble">
        ...
        function addSuggestionButtons() {
            ...
            div.innerHTML = `<div class="bubble">
                <div class="suggestion-buttons">
                    <button class="suggestion-btn" onclick="sendQuick('Get quote')">💰 Get Quote</button>
                    <button class="suggestion-btn" onclick="sendQuick('Shipping strategy')">📊 Strategy</button>
                    <button class="suggestion-btn" onclick="sendQuick('Business scaling')">📈 Scale</button>
                </div>
            </div>`;
            messagesDiv.appendChild(div);
            div.scrollIntoView({ behavior: 'smooth' });
...
def detect_intent(user_msg):
    """Detect user intent to choose response mode"""
    msg_lower = user_msg.lower()
    
    # Compliance mode - ONLY for actual dangerous goods
    dangerous_keywords = ['lithium', 'battery', 'batteries', 'chemical', 'acid', 'flammable', 'explosive', 'hazardous']
    for dk in dangerous_keywords:
        if dk in msg_lower:
            return 'compliance'
    
    # Quote intent
    quote_keywords = ['quote', 'price', 'cost', 'rate', 'how much', 'expensive', 'cheap']
    for qk in quote_keywords:
        if qk in msg_lower:
            return 'quote'
    
    # Strategy intent
    strategy_keywords = ['strategy', 'plan', 'scale', 'monthly', 'weekly', 'regular', 'optimize', 'reduce cost']
    for sk in strategy_keywords:
        if sk in msg_lower:
            return 'strategy'
    
    # Business advice intent
    business_keywords = ['business', 'company', 'grow', 'profit', 'margin', 'competitor', 'dhl', 'fedex']
    for bk in business_keywords:
        if bk in msg_lower:
            return 'business'
    
    # Beginner/onboarding intent
    beginner_keywords = ['don\'t know', 'no idea', 'first time', 'new to', 'help me', 'guide me', 'tell me']
    for bk in beginner_keywords:
        if bk in msg_lower:
            return 'beginner'
...
def update_context(user_msg, current_context):
    """Update context without dangerous goods contamination"""
    new_context = current_context.copy()
    
    # Weight extraction
    weight_match = re.search(r'(\d+)\s*(?:kg|kgs|kilogram|kilo|ton|tons)', user_msg, re.IGNORECASE)
    if weight_match:
    ...
    elif 'not urgent' in user_msg.lower():
        new_context['urgency'] = 'not urgent'
    
    # Cargo type - without auto-DG flag
    cargo_types = ['electronics', 'garments', 'clothes', 'medicine', 'food', 'machinery', 'furniture']
    for ct in cargo_types:
        if ct in user_msg.lower():
            new_context['cargo_type'] = ct
            break
...
def chat():
    ...
    if session_id not in session_memory:
        session_memory[session_id] = {
            'weight': None, 'destination': None, 'cargo_type': None, 'urgency': None
        }
    ...
    if intent == 'compliance':
        system_prompt = f"""You are Aisea, logistics advisor at PAS Freight.

⚠️ USER IS SHIPPING DANGEROUS GOODS (batteries/chemicals)

Response rules:
1. Acknowledge it's restricted cargo
2. Ask for: MSDS document, UN packaging
3. DO NOT give pricing - just compliance info
...
USER WANTS A QUOTE/PRICE.

Current shipment: {context_str}

Response rules:
- Give estimated pricing ranges
- Explain what's included
- Offer to get exact quote

Example: "For 200kg to Dubai, air: ~₹90,000 (3-5 days), sea: ~₹30,000 (25-30 days). Want me to get exact rates?"
...
USER WANTS SHIPPING STRATEGY ADVICE.

Current: {context_str}

Response rules:
- Give business-focused recommendations
- Suggest consolidation, split shipments, contract rates
- Focus on cost optimization

Example: "For 2 tons/month, negotiate contract rates. Use sea for bulk (70%), air for urgent top-up (30%). Want me to design a plan?"
...
USER ASKS BUSINESS/COMPETITIVE QUESTION.

Response rules:
- Explain PAS Freight advantages
- Compare vs competitors (DHL, FedEx) if asked
- Focus on value proposition

Example: "PAS Freight offers better rates for bulk shipments + personalized support. DHL is faster but 40-50% more expensive."
...
USER IS NEW TO SHIPPING (doesn't know anything).

Response rules:
- Be very simple and patient
- Guide step by step with 3 questions only
- Don't overwhelm with technical terms

Example: "No problem! I'll guide you. Just tell me: 1) What are you sending? 2) Approx weight? 3) Where to? I'll handle the rest."
...
- Show air vs sea difference
- Ask 1 smart follow-up question

Example: "For 300kg to Germany not urgent, sea saves 60-70%. Takes 25-30 days. Want cheapest or faster delivery?"

Be confident and helpful. Keep response short (2-3 sentences)."""
...
if __name__ == '__main__':
    print("=" * 60)
    print("✨ Aisea - Intent-Based AI")
    print("📱 Open: http://localhost:5001")
    print("🧠 Modes: Quote | Strategy | Business | Beginner | Advisor")
    print("=" * 60)
