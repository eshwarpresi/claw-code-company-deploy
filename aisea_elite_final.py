...
def detect_intent(user_msg, ctx):
    ...
    # MULTI-INTENT: Safety + Cost + Not Urgent (most important)
    if ('not urgent' in msg_lower or 'no rush' in msg_lower) and ('safe' in msg_lower or 'safety' in msg_lower) and ('cost' in msg_lower or 'price' in msg_lower or 'good pricing' in msg_lower):
        return 'multi_intent_balance'
    ...
    # Enterprise / Weekly shipments to multiple countries
    if ('weekly' in msg_lower or 'monthly' in msg_lower) and ('multiple countries' in msg_lower or 'international' in msg_lower or 'regular' in msg_lower):
        return 'enterprise'
    ...
    # Trust / complaint / delayed
    if any(word in msg_lower for word in ['lost', 'damage', 'trust', 'reliable', 'safe', 'insurance', 'claim', 'delayed', 'company delayed', 'forwarder']):
        return 'trust'
    
    # Negotiation / match price with ready intent
    if ('match price' in msg_lower or 'match rate' in msg_lower) and ('confirm' in msg_lower or 'good' in msg_lower or 'proceed' in msg_lower):
        return 'negotiation_ready'
    
    # Ready to proceed / price is good
    if any(word in msg_lower for word in ['price is good', 'confirm now', 'ready to book', 'proceed', 'finalize']):
        return 'ready'
    ...
    # Bulk
    if any(word in msg_lower for word in ['bulk', 'volume', 'frequent']):
        return 'bulk'
    ...
    # Comparison
    if any(word in msg_lower for word in ['air vs', 'split vs', 'compare', 'better option']):
        return 'comparison'
...
def chat():
    ...
    # Extract weight - FIX for Nonekg bug
    ...
    # Clean weight display - NO "Nonekg"
    weight_display = ctx.get('weight', '150') if ctx.get('weight') else '150'
    dest_display = ctx.get('destination', 'Dubai') if ctx.get('destination') else 'Dubai'
    
    session_memory[session_id] = ctx
    
    # 1. MULTI-INTENT: Safety + Cost + Not Urgent
    if intent == 'multi_intent_balance':
        return jsonify({'reply': f"Got it 👍 since it's not urgent and you want safety + good pricing:\n\n✈️ Economy air → safer handling, moderate cost\n🚢 Sea (with insurance) → lowest cost\n\nFor {weight_display}kg to {dest_display}, I'd recommend economy air for safety, or sea with insurance for savings. Which matters more?"})
    
    # 2. ENTERPRISE - Weekly shipments to multiple countries
    if intent == 'enterprise':
        return jsonify({'reply': "That's great 👍 we handle end-to-end logistics partnerships.\n\n✔️ Dedicated account manager\n✔️ Bulk pricing\n✔️ Centralized tracking system\n✔️ Documentation handling\n\nLet's understand your volume so we can set this up properly. How many kg per week?"})
    
    # 3. DESTINATION CHANGE - Preserve context, NO "Nonekg"
    if intent == 'destination_change':
        new_dest = dest_display
        if new_dest != 'Dubai':
            ctx['destination'] = new_dest
            session_memory[session_id] = ctx
        return jsonify({'reply': f"Got it 👍 changing destination to {new_dest}.\nSame {weight_display}kg electronics.\n\nDo you prefer faster delivery or lower cost?"})
    
    # 4. NEGOTIATION READY - Price is good, confirm now
    if intent == 'negotiation_ready':
        return jsonify({'reply': "Perfect 👍 I'll check the best option for you.\n\nJust share your name and WhatsApp number, and I'll arrange everything quickly.\n\nWhat's the best number to reach you?"})
    
    # 5. READY - Strong closing
    if intent == 'ready':
    ...
    # 9. URGENT
    if intent == 'urgent':
        return jsonify({'reply': f"For urgent delivery to {dest_display}:\n\n✓ Express air: 3-5 days\n✓ Priority handling\n✓ Real-time tracking\n\nShall I check flight availability?"})
    
    # 10. BUDGET
    if intent == 'budget':
        return jsonify({'reply': f"For {weight_display}kg to {dest_display}, sea freight saves 60-70% compared to air.\n\nSea: 20-30 days | Air: 3-7 days\n\nWant me to check current sea freight rates?"})
    ...
    if intent == 'comparison':
        return jsonify({'reply': f"Good question 👍\n\n✔️ Full air → fastest ({weight_display}kg, 3-5 days), highest cost\n✔️ Split (air + sea) → balances cost & speed\n\nFor {weight_display}kg, split option often gives better value if not urgent. Want me to check both options?"})
    ...
    # 15. SHIPPING
    if intent == 'shipping':
        if weight_display and dest_display:
            return jsonify({'reply': f"For {weight_display}kg to {dest_display}:\n\n✈️ Air: 3-7 days (faster, higher cost)\n🚢 Sea: 20-30 days (slower, saves 60-70%)\n\nWhich matters more - speed or cost?"})
        elif weight_display:
            return jsonify({'reply': f"For {weight_display}kg shipment - where is it going?\n\n✈️ Air: 3-7 days\n🚢 Sea: 20-30 days\n\nShare destination for better recommendation."})
        elif dest_display:
            return jsonify({'reply': f"To {dest_display} - what's the approximate weight?\n\n✈️ Air: 3-7 days\n🚢 Sea: 20-30 days\n\nWeight helps me recommend the best option."})
        else:
...
if __name__ == '__main__':
    print("=" * 60)
    print("Aisea - ELITE 10/10 FINAL Chatbot")
    print("Open: http://localhost:5001")
    print("Fixed: Multi-intent | No Nonekg bug | Smart closing | Enterprise handling")
    print("=" * 60)
