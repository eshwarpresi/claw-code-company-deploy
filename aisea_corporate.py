...
def get_intent_and_context(user_msg, ctx):
    msg_lower = user_msg.lower()
    
    # DETECT DESTINATION CHANGE (CRITICAL)
    change_patterns = [
        r'change to (\w+)', r'update to (\w+)', r'switch to (\w+)', 
        r'actually (\w+)', r'not (\w+)', r'change destination to (\w+)'
    ]
    for pattern in change_patterns:
        match = re.search(pattern, msg_lower)
        if match:
            new_dest = match.group(1).title()
            if new_dest != ctx.get('destination'):
                ctx['destination_changed'] = True
                ctx['destination'] = new_dest
                return 'destination_changed', new_dest
    
    # DETECT INTENT
    # Hot lead / ready to book
    if any(word in msg_lower for word in ['confirm now', 'ready to book', 'lock it', 'proceed', 'book it', 'finalize']):
        return 'hot_lead', None
    
    # Expert advice request
    if 'what would you do' in msg_lower or 'what would you recommend' in msg_lower or 'suggest me' in msg_lower:
        return 'expert_advice', None
    ...
    # Cost focus
    if any(word in msg_lower for word in ['cheapest', 'budget', 'save money', 'affordable', 'low cost']):
        return 'cost_focus', None
    
    # Trust / safety concern
    if any(word in msg_lower for word in ['safe', 'damage', 'insurance', 'protect', 'reliable']):
        return 'trust_concern', None
    
    # Enterprise / bulk
    if any(word in msg_lower for word in ['bulk', 'monthly', 'weekly', 'regular', 'volume', 'enterprise']):
    ...
    # Human handoff
    if any(word in msg_lower for word in ['human', 'talk to person', 'real person', 'customer care']):
        return 'human_handoff', None
    
    # Regular query - extract details
    return 'regular', None

def get_smart_response(intent, ctx, user_msg):
    weight = ctx.get('weight', 'your')
    dest = ctx.get('destination', 'destination')
    cargo = ctx.get('cargo_type', 'goods')
    
    # DESTINATION CHANGED - Acknowledge and adapt
    if intent == 'destination_changed':
        return f"Got it 👍 I've updated the destination to {dest}. Based on this change, for {weight}kg {cargo}, here's what I recommend: {'Express air (3-5 days) if urgent, or sea (20-30 days) for cost savings.' if ctx.get('urgency') == 'urgent' else 'Sea freight for cost savings (20-30 days), or air if speed matters.'} Want me to check rates for {dest}?"
    
    # HOT LEAD - Strong closing (CONVERSION MOMENT)
    if intent == 'hot_lead':
        return f"Perfect 👍 I'll lock this for you now. Just share your name and WhatsApp number - I'll get our team to confirm immediately and handle everything from pickup to delivery. What's your number?"
    
    # EXPERT ADVICE - Consultant-level response
    if intent == 'expert_advice':
        if cargo == 'electronics':
            return "If it was my shipment, I'd split it: critical/high-value items via air for safety and speed, remaining via sea for cost savings. This balances risk and budget. Want me to help you plan this split?"
        else:
            return "If it was my shipment, I'd choose sea freight for cost savings unless delivery time is critical. Sea saves 60-70% but takes 20-30 days. Air is faster at 3-7 days but costs more. What's more important to you - speed or budget?"
    
    # URGENT
    if intent == 'urgent':
        return f"For urgent delivery to {dest}, air freight is your fastest option - 3-5 days. I can check immediate flight availability for your {weight}kg {cargo}. Shall I proceed?"
    
    # NOT URGENT
    if intent == 'not_urgent':
        return f"Since you're not in a hurry, sea freight gives you the best value - saves 60-70% vs air. Takes 20-30 days to {dest}. Want me to check current sea rates for your {weight}kg {cargo}?"
    
    # COST FOCUS
    if intent == 'cost_focus':
        return f"For budget-friendly shipping to {dest}, sea freight is your best bet at 60-70% less than air. Takes 20-30 days. Want me to get you the current sea rate for {weight}kg {cargo}?"
    
    # TRUST CONCERN
    if intent == 'trust_concern':
        return f"Your concern is valid 👍 We handle shipments with: insurance coverage, real-time tracking, and dedicated support. For {cargo}, we use ESD-safe packaging and anti-static handling. Need specific details about protection?"
    
    # ENTERPRISE / BULK
    if intent == 'enterprise':
        return f"For regular shipments, we offer volume discounts, dedicated account manager, and priority space. What's your estimated monthly volume? I'll prepare a customized pricing proposal for you."
    
    # TRACKING
    if intent == 'tracking':
        return "Please share your tracking ID or reference number - I'll check the status for you right away."
    ...
    # REGULAR - Extract info or use AI
    return None

def extract_shipment_details(user_msg, ctx):
    msg_lower = user_msg.lower()
    ...
    # Destination extraction (only if not changed via intent)
    if not ctx.get('destination_changed'):
        dest_match = re.search(r'(?:to|for)\s+([A-Z][a-z]+|[A-Z]+)', msg_lower)
        if dest_match:
            ctx['destination'] = dest_match.group(1).title()
    
    # Cargo type
    cargo_map = {'electronics': 'electronics', 'garments': 'garments', 'fragile': 'fragile', 'medicine': 'medicine', 'machinery': 'machinery'}
    for key, value in cargo_map.items():
        if key in msg_lower:
            ctx['cargo_type'] = value
            break
    
    # Reset destination_changed flag
    ctx['destination_changed'] = False
...
def chat():
    ...
    if session_id not in session_memory:
        session_memory[session_id] = {
            'cargo_type': None, 'destination': None, 'weight': None, 
            'urgency': None, 'destination_changed': False
        }
    
    ctx = session_memory[session_id]
    
    # Detect intent first (handles destination changes)
    intent, intent_value = get_intent_and_context(user_msg, ctx)
    
    # Extract shipment details
    ctx = extract_shipment_details(user_msg, ctx)
    
    # Get smart response based on intent
    smart_response = get_smart_response(intent, ctx, user_msg)
    if smart_response:
        session_memory[session_id] = ctx
        return jsonify({'reply': smart_response})
    
    # For messy input or other queries, use AI with context
    context_summary = f"Destination: {ctx.get('destination', 'unknown')}, Weight: {ctx.get('weight', 'unknown')}kg, Cargo: {ctx.get('cargo_type', 'general')}, Urgency: {ctx.get('urgency', 'normal')}"
...
- Use the context - don't ask for info already given
- If user changed destination, acknowledge it
- End with a question to move conversation forward
- Never repeat the same recommendation

Company: PAS Freight, Contact: +91 90361 01201

Be helpful, direct, and smart."""
...
if __name__ == '__main__':
    print("=" * 60)
    print("Aisea - CORPORATE-READY Production Chatbot")
    print("Open: http://localhost:5001")
    print("Features: Context-aware | Non-repetitive | Intent-driven | Strong closing")
    print("=" * 60)
