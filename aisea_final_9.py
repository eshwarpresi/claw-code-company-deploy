...
def extract_shipments(text):
    pattern = r'(\d+)\s*kg\s*(?:to\s*)?([a-zA-Z\s]+?)(?=\d+kg|$|\.|\,|and)'
...
def chat():
    ...
    # Contract pricing
    if 'contract' in user_msg or 'regular shipment' in user_msg or 'volume' in user_msg:
    ...
    # Bulk shipment validation
    weight_match = re.search(r'(\d+(?:\.\d+)?)\s*(?:kg|kgs|kilogram)', user_msg)
    if weight_match:
        weight = float(weight_match.group(1))
        if weight > 10000:
            reply = "📦 For bulk shipments over 10,000 kg, please contact our team directly for special pricing.\n\n📞 Call +91 90361 01201 or share your number for a callback."
            return jsonify({'reply': reply})
    
    # Handle corrections with "actually", "instead", "correction"
    correction_keywords = ['actually', 'instead', 'correction', 'change', 'not', 'wait']
    is_correction = any(word in user_msg for word in correction_keywords)
    
    # Extract weight (ALWAYS update with latest)
    if weight_match:
        session_data['weight'] = float(weight_match.group(1))
    
    # Extract destination with validation
    dest_match = None
    ...
    # Extract cargo type (ALWAYS update with latest)
    cargo_types = ['electronics', 'garments', 'clothes', 'machinery', 'documents', 'spare parts', 'furniture', 'food', 'pharma', 'chemicals']
    for cargo in cargo_types:
        if cargo in user_msg:
            session_data['cargo'] = cargo
            break
    
    # If no specific cargo detected, keep existing or set to general
    if not any(cargo in user_msg for cargo in cargo_types):
        if session_data['cargo'] is None:
            session_data['cargo'] = 'general'
    ...
    # Handle urgency
    urgency_keywords = ['urgent', 'fast', 'quick', 'asap', 'emergency', 'next friday', 'this week', 'immediate']
    is_urgent = any(word in user_msg for word in urgency_keywords)
    ...
    # ========== QUOTE LOGIC (ONLY WHEN DATA IS AVAILABLE) ==========
    ...
    # Check if we have enough data for a quote
    if has_weight and has_dest:
        ...
        if cargo == 'electronics':
            ...
            reply += "👕 Sea freight is economical for garments. Air is faster but costs more.\n\n"
        elif cargo == 'pharma':
            reply += "💊 Pharma shipments require temperature control. Air freight with cold chain available.\n\n"
        
        if is_urgent:
            reply += "🚀 Since you need urgent delivery, air freight is your best option.\n\n"
        
        reply += "Want me to lock the best rate? Share your phone number."
        
        return jsonify({'reply': reply})
    
    # ========== INTELLIGENT MISSING INFO ==========
    ...
    # Handle services
    if 'service' in user_msg or 'offer' in user_msg:
        reply = """📦 Our services:
...
Which service do you need?"""
    ...
    # Handle address
    if 'address' in user_msg or 'location' in user_msg or 'office' in user_msg:
...
if __name__ == '__main__':
    ...
    print("✅ Fixed: Context | Validation | Corrections | Special handlers")
    print("=" * 60)
