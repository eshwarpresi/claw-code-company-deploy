...
<html>
<head>
    <title>Aisea - PAS Freight Adaptive AI</title>
    <meta charset="UTF-8">
    ...
    <div id="pasfreight-chat-window">
        <div class="chat-header">
            <div class="header-info">
                ...
                <div class="header-text">
                    <h3>Aisea</h3>
                    <p>Adaptive Logistics AI • 24/7</p>
                </div>
            </div>
        ...
        <div class="chat-messages" id="chatMessages">
            <div class="message bot-message">
                <div class="bubble">Hi, I'm Aisea - adaptive logistics strategist. I consider profit, speed, risk, and business impact.</div>
            </div>
            <div class="message bot-message">
                <div class="bubble">
        ...
        function addSuggestionButtons() {
            ...
            div.innerHTML = `<div class="bubble">
                <div class="suggestion-buttons">
                    <button class="suggestion-btn" onclick="sendQuick('Optimize profit')">💰 Optimize</button>
                    <button class="suggestion-btn" onclick="sendQuick('Risk analysis')">⚠️ Risk</button>
                    <button class="suggestion-btn" onclick="sendQuick('Business impact')">📈 Impact</button>
                </div>
            </div>`;
            messagesDiv.appendChild(div);
            div.scrollIntoView({ behavior: 'smooth' });
...
def calculate_adaptive_recommendation(margin, weight, urgency=None, customer_value_factor=None, risk_tolerance=None):
    """Adaptive recommendation engine - NOT rigid rules"""
    
    air_profit = margin - AIR_COST_PER_KG
    sea_profit = margin - SEA_COST_PER_KG
    
    # Business factors that can override pure profit math
    business_value_multiplier = 1.0
    
    # Customer acquisition value - air can bring more customers
    if customer_value_factor:
        business_value_multiplier += customer_value_factor / 100
    
    # Urgency factor - stockout cost
    if urgency == 'high':
        business_value_multiplier += 0.3
    
    # Risk tolerance
    if risk_tolerance == 'low':
        business_value_multiplier -= 0.2
    
    adjusted_air_value = air_profit * business_value_multiplier
    adjusted_sea_value = sea_profit
    
    return {
        'air_profit_raw': air_profit,
        'sea_profit_raw': sea_profit,
        'air_viable_math': air_profit > 0,
        'air_viable_business': adjusted_air_value > adjusted_sea_value,
        'business_multiplier': business_value_multiplier,
        'recommendation': 'air' if adjusted_air_value > adjusted_sea_value else 'sea',
        'confidence': 'strong' if abs(adjusted_air_value - adjusted_sea_value) > 50 else 'moderate'
    }
...
def detect_intent(user_msg):
    ...
    if 'risk' in msg_lower or 'damage' in msg_lower or 'delay' in msg_lower:
        return 'risk'
    
    if 'customer' in msg_lower or 'competitive' in msg_lower or 'market' in msg_lower:
        return 'business_impact'
    
    profit_keywords = ['profit', 'margin', 'optimize', 'maximize', 'roi']
    for p in profit_keywords:
        if p in msg_lower:
            return 'profit'
    
    strategy = ['strategy', 'scale', 'monthly', 'weekly', 'regular']
    for s in strategy:
        if s in msg_lower:
            return 'strategy'
    
    quote = ['quote', 'price', 'cost', 'rate', 'how much']
    for q in quote:
        if q in msg_lower:
            return 'quote'
    
    beginner = ["don't know", "no idea", "first time", "new to"]
    for b in beginner:
        if b in msg_lower:
            return 'beginner'
...
def update_context(user_msg, ctx):
    ...
    if 'urgent' in user_msg.lower():
        new_ctx['urgency'] = 'high'
    
    if 'customer' in user_msg.lower() or '40% more' in user_msg.lower():
        new_ctx['customer_value_factor'] = 40
...
def chat():
    ...
    if session_id not in session_memory:
        session_memory[session_id] = {'weight': None, 'destination': None, 'cargo': None, 'urgency': None, 'margin': None, 'customer_value_factor': None}
    ...
    if intent == 'risk':
        prompt = f"""You are Aisea, adaptive logistics strategist.

User asks about risk assessment.

Consider: damage risk (electronics 3-5% in sea), customs delay (7-10 days possible), seasonality impact.

Current: {context_display}

Give balanced risk analysis. Include mitigation strategies. Keep response short (3-4 lines)."""
    
    elif intent == 'business_impact':
        prompt = f"""You are Aisea, adaptive logistics strategist.

User asks about business impact beyond pure cost.

Consider: customer lifetime value, competitive advantage, stockout costs, brand perception.

CRITICAL: If air reduces margin but increases customers by 40%, it MAY be viable.

Current: {context_display}

Give nuanced recommendation considering business factors. Be specific with percentages."""
    
    elif intent == 'profit' and ctx.get('margin'):
        profit_math = calculate_adaptive_recommendation(
            ctx['margin'], 
            ctx.get('weight', 1), 
            ctx.get('urgency'),
            ctx.get('customer_value_factor'),
            None
        )
        
        profit_advice = f"""Based on your margin (Rs.{ctx['margin']}/kg):
• Raw air profit: Rs.{profit_math['air_profit_raw']}/kg
• Raw sea profit: Rs.{profit_math['sea_profit_raw']}/kg
• Business multiplier: {profit_math['business_multiplier']:.1f}x
• Adjusted recommendation: {profit_math['recommendation'].upper()}
• Confidence: {profit_math['confidence']}

This considers customer value and urgency, not just raw math."""
...
Give ONE clear recommendation with reasoning. If business factors outweigh cost, explain why. Keep response short (3-4 lines)."""
...
Think dynamically based on:
- Demand pattern (seasonal/stable)
- Inventory turnover
- Cash flow priority
- Customer expectations

Avoid fixed ratios like 70/30 unless justified by data. Keep response short (3-4 lines)."""
...
Give decision advice that balances profit, speed, and business impact.
Avoid rigid rules. Consider trade-offs.
Be confident but nuanced.
...
if __name__ == '__main__':
    print("=" * 60)
    print("Aisea - Adaptive Logistics AI (10/10 Target)")
    print("Open: http://localhost:5001")
    print("Features: Adaptive logic | Business impact | Risk analysis")
    print("=" * 60)
