...
EMAIL_CONFIG = {
    ...
    "receiver_email": "shivu@pasfreight.com"
}

def send_email_lead(name, email, phone, service_type, details):
    try:
        subject = f"New Lead: {name} - {service_type}"
        body = f"""
        New Lead from PAS Freight Chatbot
        ...
        server.quit()
        print(f"Email sent to shivu@pasfreight.com for {name}")
        return True
...
<head>
    ...
    <style>
        * {
        ...
        /* Action Buttons - Get Quote & Talk to Expert */
        .action-buttons {
            display: flex;
            gap: 12px;
            margin: 16px 0;
...
def submit_quote():
    ...
    # Send email
    send_email_lead(name, email, phone, "Quote Request", "User requested a quote via chatbot")
...
def chat():
    ...
    if any(word in msg_lower for word in ['ready', 'book', 'proceed', 'finalize']):
        intent = 'convert'
    elif ('not urgent' in msg_lower or 'not in a hurry' in msg_lower):
        intent = 'balanced'
    elif any(word in msg_lower for word in ['cheapest', 'budget', 'save money']):
        intent = 'cost'
    elif any(word in msg_lower for word in ['urgent', 'fast', 'quick', 'express']):
        intent = 'urgent'
    elif any(word in msg_lower for word in ['bulk', 'monthly', 'weekly', 'regular']):
        intent = 'bulk'
    elif any(word in msg_lower for word in ['customs', 'clearance', 'duty']):
        intent = 'customs'
    ...
    if intent == 'convert':
        reply = f"""Perfect 👍 I'll arrange this for you right away.
...
What's the best number to reach you?"""
        ...
        reply = f"""I understand 👍 You want reliability without overspending.
...
Since time isn't urgent, sea freight gives you maximum value. Would you like me to check the exact sea freight option for you?"""
        ...
        reply = f"""For {weight}kg to {dest}, sea freight saves 60-70% compared to air.
...
Our team proactively reviews documents to avoid delays. Need help with a specific shipment?"""
        ...
        reply = f"""Electronics require special handling 👍
...
Be helpful, confident, and premium."""
        ...
        try:
            ...
            reply = "Tell me what you'd like to ship - I'll help you figure out the best solution."
...
if __name__ == '__main__':
    print("=" * 60)
    print("Aisea - Final Version with Quote Form & WhatsApp")
    print("Open: http://localhost:5001")
    print("Features: Get Quote Form | WhatsApp Expert | Email to shivu@pasfreight.com")
    print("=" * 60)
