...
# Urgency lines - rotated to avoid repetition
URGENCY_LINES = [
    "Rates fluctuate based on space availability",
    "Limited slots for this route this week",
    "Current pricing is competitive today",
    "Space is filling fast for this destination",
    "These rates are valid for today only"
]

# Closing lines - structured for conversion
CLOSING_LINES = [
    "Share your name + number - our team will call you in 10 minutes with exact rates.",
    "Want me to lock today's best rate? Share your contact details.",
    "I can secure this rate for you. Just share your name and phone number.",
    "Let me get you the exact quote. Share your details and our team will reach out immediately."
]
...
def chat():
    ...
    if session_id not in session_memory:
        session_memory[session_id] = {'urgency_index': 0, 'closing_index': 0}
    
    # Rotate urgency and closing lines
    urgency_index = session_memory[session_id].get('urgency_index', 0) % len(URGENCY_LINES)
    closing_index = session_memory[session_id].get('closing_index', 0) % len(CLOSING_LINES)
    
    urgency_line = URGENCY_LINES[urgency_index]
    closing_line = CLOSING_LINES[closing_index]
    
    session_memory[session_id]['urgency_index'] = urgency_index + 1
    session_memory[session_id]['closing_index'] = closing_index + 1
    
    system_prompt = f"""You are Aisea, a sales-driven logistics expert for PAS Freight.

RULES:
- Keep responses SHORT (2-3 lines max)
- ALWAYS end with a closing line
- Give ESTIMATED RATES first, then close
- Use urgency naturally: "{urgency_line}"
...
Be direct, give rates first, then close."""
    ...
    try:
        ...
        return jsonify({'reply': f'✈️ Air: ₹320-400/kg (3-5 days)\n🚢 Sea: ₹80-120/kg (20-30 days)\n{random.choice(URGENCY_LINES)}\n{random.choice(CLOSING_LINES)}'})

if __name__ == '__main__':
    print("=" * 60)
    print("Aisea - FINAL 10/10 Version")
    print("Open: http://localhost:5001")
    print("Features: Rotating urgency | Strong closing | Rate-first responses")
    print("=" * 60)
