...
function showOptions(service) {
    ...
    if (service === 'import' || service === 'export') {
        ...
        html = '<div class="sub-btns"><button class="sub-btn" data-mode="domestic">🇮🇳 Domestic</button><button class="sub-btn" data-mode="international">🌍 International</button></div>';
    }
...
function selectService(service) {
    ...
    if (service === 'customs') {
        addMsg('I need CUSTOMS CLEARANCE', true);
        addMsg('📋 For Customs Clearance, please contact:\n📞 Call us: +91 63615 26664\n\nOr share your details below and we will call you back.', false);
        step = 'name';
        addMsg('Please share your name:', false);
        return;
    }
    
    if (service === 'domestic' || service === 'courier') {
        addMsg('I need ' + names[service] + ' service', true);
        addMsg('📞 For ' + names[service] + ' service, please call: +91 96116 60204\n\nOr share your details below and we will call you back.', false);
        step = 'name';
        addMsg('Please share your name:', false);
        return;
    }
...
function selectMode(mode) {
    ...
    addMsg('📞 For ' + mode.toUpperCase() + ' freight, please call: +91 90716 60066\n\nOr share your details below and we will call you back.', false);
    step = 'name';
    addMsg('Please share your name:', false);
}
...
async function sendMsg() {
    ...
    if (step === 'phone') {
        ...
        await fetch('/save_lead', {
            ...
            body: JSON.stringify({ service: currentService, mode: currentMode, name: userName, email: userEmail, phone: userPhone })
        });
        
        addMsg('✅ Thank you ' + userName + '! Our team will call you back shortly.', false);
        
        if (currentService === 'customs') {
            addMsg('📞 For urgent customs queries: +91 63615 26664', false);
        } else if (currentService === 'domestic' || currentService === 'courier') {
            addMsg('📞 For immediate assistance: +91 96116 60204', false);
        } else {
            addMsg('📞 For immediate assistance: +91 90716 60066', false);
        }
...
def save_lead():
    ...
    print(f"LEAD: {data.get('name')} - {data.get('service')} - {data.get('mode')}")
...
if __name__ == '__main__':
    ...
    print("📞 Import/Export: 90716 60066")
    print("📞 Customs: 63615 26664")
    print("📞 Domestic/Courier: 96116 60204")
    print("=" * 50)
