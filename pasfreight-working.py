...
<body>
    <div class="chat">
        ...
        <div class="messages" id="messages">
            <div class="msg bot"><div class="bubble">Namaste! Welcome to PAS Freight. How can I assist you with freight forwarding, customs clearance, or logistics today?</div></div>
        </div>
...
def chat():
    ...
    company_info = """PAS Freight Services Pvt Ltd is a logistics company in Bangalore, India specializing in:
    ...
    Address: Site No:171, Arkavathey Layout, Jakkur-BDA, Bangalore - 560092
    WCA Member ID: 115513, GLA Member ID: 1166251"""
    
    try:
        # Use the correct OpenRouter model format
        response = openai.ChatCompletion.create(
            model="openai/gpt-4o-mini",  # Correct format for OpenRouter
            messages=[
                {"role": "system", "content": f"You are PAS Freight AI Assistant. Company info: {company_info}. Be helpful and professional."},
                {"role": "user", "content": user_msg}
            ],
            max_tokens=500,
            temperature=0.7,
            headers={
                "HTTP-Referer": "https://pasfreight.com",
                "X-Title": "PAS Freight AI Assistant"
            }
        )
        reply = response['choices'][0]['message']['content']
        print(f"Success: {reply[:100]}")  # Debug
        return jsonify({'reply': reply})
    except Exception as e:
        error_msg = str(e)
        print(f"ERROR: {error_msg}")
        # Return a helpful response even if API fails
        return jsonify({'reply': f'Thank you for your inquiry. For immediate assistance with {user_msg}, please contact PAS Freight at +91 9071660066 or email shivu@pasfreight.com'})
