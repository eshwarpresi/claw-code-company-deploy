...
def detect_intent(user_msg, ctx):
    ...
    if any(word in msg_lower for word in ['ready', 'book', 'make it simple', 'proceed', 'finalize']):
        return 'ready'
    
    # Intent: Competitor/Cheaper alternative
    if any(word in msg_lower for word in ['cheaper', 'competitor', 'dhl', 'fedex', 'other company', 'better rate']):
        return 'competitor'
...
def chat():
    ...
    if session_id not in session_memory:
        session_memory[session_id] = {'cargo_type': None, 'destination': None, 'weight': None, 'intent': None, 'origin': None}
    
    ctx = session_memory[session_id]
    intent = detect_intent(user_msg, ctx)
    ctx['intent'] = intent
    
    # Extract shipment info without over-asking
    ...
    # READY TO BOOK - Strong closing
    if intent == 'ready':
        response = """Perfect 👍 I'll keep it simple.

Just share your contact number or WhatsApp,
and our team will take care of everything from pickup to delivery.
...
Pricing can vary, but what matters is reliability and handling, especially for shipments like yours.

A slightly lower rate can sometimes lead to delays or issues.

Let me see if I can match a better option while keeping service quality high.

What's your target price range?"""
    
    # HIGH VALUE / TRUST (Strong version)
    elif intent == 'trust':
        response = """That's a high-value shipment 👍 completely understandable.
...
Would you like me to check the best secure option for you?"""
    ...
    # COST
        ...
        response = f"""If cost is your priority 👍

🚢 Sea freight is more economical, but takes longer (20-30 days).

Since you mentioned {ctx.get('cargo_type', 'your shipment')}, if it's not urgent, sea can save you 60-70%.

For {ctx.get('weight', 'your')}kg to {ctx.get('destination', 'destination')}, I can check both options.

What's your priority - maximum savings or faster delivery?"""
    
    # URGENT
    elif intent == 'urgent':
        response = f"""For urgent delivery to {ctx.get('destination', 'your destination')} 👍
...
For {ctx.get('weight', 'your')}kg, I can check flight availability right now.
...
Need help with a specific shipment? I can connect you with our customs expert."""
    ...
    # ELECTRONICS - Not urgent, low risk
    elif intent == 'electronics' and 'not urgent' in user_msg.lower():
        response = f"""Since it's not urgent 👍 and you want to reduce risk,

we can look at:
• Secure sea freight with proper packaging + insurance
• Economy air for safer handling (faster than sea, cheaper than express)

For {ctx.get('weight', 'your')}kg of {ctx.get('cargo_type', 'electronics')} to {ctx.get('destination', 'destination')},

I'll help you choose the best balance of cost and safety.

Would you like me to check both options?"""
    
    # ELECTRONICS - Standard
    elif intent == 'electronics':
        response = f"""Electronics need extra care 👍
...
Shall I check the best air freight option for you?"""
    ...
    # ELECTRONICS with cost priority
    elif intent == 'electronics_cost':
        response = f"""For {ctx.get('cargo_type', 'electronics')} on a budget 👍

While air is recommended for safety, sea freight can work if:
• Goods are well-packaged
• Not extremely urgent
• You accept slightly longer transit (20-30 days)

For {ctx.get('weight', 'your')}kg, sea could save you significant cost.

Want me to check both air and sea options for comparison?"""
    
    # ELECTRONICS urgent
    elif intent == 'electronics_urgent':
        response = f"""For urgent {ctx.get('cargo_type', 'electronics')} 👍
...
Shall I check the best available rates?"""
    
    # GENERAL - Use AI for other queries
    else:
...
- Only ask for missing info, never repeat
...
if __name__ == '__main__':
    print("=" * 60)
    print("Aisea - FINAL 9.5/10 Logistics Consultant")
    print("Open: http://localhost:5001")
    print("Fixed: Response accuracy | Strong closing | Trust building | No over-questioning")
    print("=" * 60)
