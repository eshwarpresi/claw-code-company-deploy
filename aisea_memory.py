    ...
    <div id="pasfreight-chat-window">
        ...
        <div class="chat-messages" id="chatMessages">
            <div class="message bot-message">
                <div class="bubble">Hi, I'm Aisea. Share your shipment details - weight, destination, cargo type.</div>
            </div>
            <div class="message bot-message">
                <div class="bubble">
...
CRITICAL RULES:
1. NEVER ask for information the user already provided. Track this context:
   - Weight: {memory.get('weight', 'unknown')}
   - Destination: {memory.get('destination', 'unknown')}
   - Cargo type: {memory.get('cargo_type', 'unknown')}
   - Urgency: {memory.get('urgency', 'unknown')}

2. When user says "change to X" or "make it Y", UPDATE the context, don't ask again.

3. Keep responses SHORT (2-3 sentences max). No paragraphs.

4. For dangerous goods (lithium, chemicals, batteries, liquids):
   - Switch to COMPLIANCE MODE immediately
   - Ask for MSDS, UN packaging
   - Do NOT give pricing

5. For normal cargo: Give decision guidance with:
   - Air vs Sea cost difference
   - Time difference
   - Recommendation based on value/urgency

6. Sales: Only ask for phone number AFTER providing value, not every message.

Current shipment context:
📦 Weight: {memory.get('weight', 'Not provided')}
📍 Destination: {memory.get('destination', 'Not provided')}
📋 Cargo: {memory.get('cargo_type', 'Not provided')}
⏱️ Urgency: {memory.get('urgency', 'Not provided')}

Update these values based on the user's message. If user says "200 kg", update weight to 200. If "change to Germany", update destination to Germany.

Respond as a confident logistics advisor. Short, sharp, helpful."""
    ...
    try:
        ...
        # Update memory based on user message (simple extraction)
        import re
        weight_match = re.search(r'(\d+)\s*(?:kg|kgs|kilogram)', user_msg, re.IGNORECASE)
        if weight_match:
            memory['weight'] = weight_match.group(1)
        
        dest_match = re.search(r'(?:to|for|from)\s+([A-Z][a-z]+)', user_msg)
        if dest_match:
            memory['destination'] = dest_match.group(1)
        
        if 'urgent' in user_msg.lower() or 'fast' in user_msg.lower():
            memory['urgency'] = 'urgent'
        elif 'not urgent' in user_msg.lower() or 'slow' in user_msg.lower():
            memory['urgency'] = 'not urgent'
        
        cargo_types = ['electronics', 'garments', 'chemicals', 'batteries', 'lithium', 'medicine', 'food']
        for ct in cargo_types:
            if ct in user_msg.lower():
                memory['cargo_type'] = ct
...
if __name__ == '__main__':
    print("=" * 60)
    print("✨ Aisea - Memory-Powered 9.5/10")
    print("📱 Open: http://localhost:5001")
    print("🧠 Features: Context memory | No repeat questions | Update handling")
    print("=" * 60)
