...
<html>
<head>
    <title>Aisea - PAS Freight AI Expert</title>
    <meta charset="UTF-8">
    ...
    <div id="pasfreight-chat-window">
        ...
        <div class="chat-messages" id="chatMessages">
            <div class="message bot-message">
                <div class="bubble">Hi, I'm Aisea - your logistics expert. Ask me about pricing, strategy, or business scaling.</div>
            </div>
            <div class="message bot-message">
                <div class="bubble">
        ...
        function addSuggestionButtons() {
            ...
            div.innerHTML = `<div class="bubble">
                <div class="suggestion-buttons">
                    <button class="suggestion-btn" onclick="sendQuick('Get quote')">💰 Quote</button>
...
def detect_intent(user_msg):
    ...
    # Compliance - ONLY actual dangerous goods
    ...
    # Business/margin
    business = ['margin', 'profit', 'business', 'company', 'grow', 'competitor', 'dhl', 'fedex']
    for b in business:
        if b in msg_lower:
            return 'business'
...
def chat():
    ...
    if session_id not in session_memory:
        session_memory[session_id] = {'weight': None, 'destination': None, 'cargo': None, 'urgency': None, 'margin': None}
    
    session_memory[session_id] = update_context(user_msg, session_memory[session_id])
    ctx = session_memory[session_id]
    intent = detect_intent(user_msg)
    
    context_display = f"📦 {ctx.get('weight', '?')}kg → {ctx.get('destination', '?')} | {ctx.get('cargo', 'general')}"
    if ctx.get('margin'):
        context_display += f" | Margin: ₹{ctx.get('margin')}/kg"
    if ctx.get('urgency') == 'urgent':
        context_display += " | URGENT"
    
    if intent == 'compliance':
        prompt = f"""You are Aisea, logistics expert.

⚠️ DANGEROUS GOODS: User is shipping {ctx.get('cargo', 'hazardous items')}

RULES:
- Do NOT give pricing
- Explain restrictions and requirements
- Ask for MSDS and UN packaging

Keep response short (3 lines max). Be helpful but firm."""
...
Give estimated pricing with breakdown. Keep response SHORT (3 lines max).

Example format: "Air: ₹X/kg (3-5 days), Sea: ₹Y/kg (25-30 days). Want exact rates?""
    
    elif intent == 'strategy':
        prompt = f"""You are Aisea, logistics expert.

User wants shipping strategy for: {context_display}

Think like a consultant:
- Suggest split shipments (70% sea + 30% air)
- Recommend consolidation for regular shipments
- Talk about contract rates for bulk

Keep response SHORT (3-4 lines). Be practical and actionable."""
    
    elif intent == 'business':
        prompt = f"""You are Aisea, logistics expert.

User asks business/competitive question about: {user_msg}

Think about margin, profit, and ROI:
- Calculate if air makes sense given their margin
- Compare PAS Freight vs DHL/FedEx
- Focus on value proposition

Keep response SHORT (3-4 lines). Be confident."""
...
Guide them simply with 3 questions only:
1. What are you sending?
2. Approx weight?
3. Where to?

Keep response warm and encouraging. Short (2-3 lines)."""
    
    else:  # advisor
        prompt = f"""You are Aisea, logistics expert.

Current shipment: {context_display}

Give decision advice with reasoning. Keep response SHORT (3 lines max).

Example: "For 500kg electronics weekly, 70% sea + 30% air balances cost and speed. Want me to design a plan?""
...
if __name__ == '__main__':
    print("=" * 60)
    print("✨ Aisea - Expert Logistics AI (Margin-aware)")
    print("📱 Open: http://localhost:5001")
    print("🧠 Features: Margin logic | Split shipments | Business advice")
    print("=" * 60)
