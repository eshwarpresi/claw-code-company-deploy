...
def get_smart_response(ctx, user_msg):
    ...
    if range_match:
        ...
        return f"Understood 👍 Let's take {int((int(low)+int(high))/2)}kg as estimate. For {cargo} to {dest}, {'air is best' if urgency == 'urgent' else 'sea saves you 60-70%'}. Need exact rates?"
    ...
    if 'if this was your shipment' in msg_lower or 'what would you do' in msg_lower:
        if cargo == 'electronics':
            ...
            return f"If this was my shipment, I'd choose {'air' if urgency == 'urgent' else 'sea'}. {'Air gets it there in 3-5 days' if urgency == 'urgent' else 'Sea saves you 60-70%'}."
    
    if weight != 'your' and dest and cargo != 'goods':
        if urgency == 'urgent':
            return f"For {weight}kg {cargo} to {dest}, use air freight. 3-5 days delivery. Need current rates?"
        else:
