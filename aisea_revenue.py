...
def detect_intent(user_msg, ctx):
    ...
    # Intent: Not urgent, no risk, no overspend
    if ('not urgent' in msg_lower or 'not in a hurry' in msg_lower) and ('risk' in msg_lower or 'damage' in msg_lower) and ('overspend' in msg_lower or 'budget' in msg_lower):
        return 'balanced'
    
    # Intent: Match price / negotiation
    if any(word in msg_lower for word in ['match price', 'match rate', 'negotiate', 'can you match', 'better price', 'discount']):
        return 'negotiate'
    
    # Intent: Responsibility / Liability
    if any(word in msg_lower for word in ['responsibility', 'liable', 'who is responsible', 'accountable', 'claim']):
        return 'responsibility'
    
    # Intent: Guarantee / Delivery by date
    if any(word in msg_lower for word in ['guarantee', 'delivery by', 'by friday', 'by monday', 'by tomorrow', 'exact date']):
        return 'guarantee'
    ...
    # Intent: Safety/Trust/High Value
    if any(word in msg_lower for word in ['safe', 'damage', 'insurance', 'protect', 'reliable', 'trust', 'high value', 'lakh', 'crore']):
        return 'trust'
    
    # Intent: Ready to book
    if any(word in msg_lower for word in ['ready', 'book', 'make it simple', 'proceed', 'finalize', 'go ahead']):
        return 'ready'
...
def chat():
    ...
    if session_id not in session_memory:
        session_memory[session_id] = {'cargo_type': None, 'destination': None, 'weight': None, 'intent': None, 'origin': None, 'last_discussed': None}
    
    ctx = session_memory[session_id]
    intent = detect_intent(user_msg, ctx)
    ctx['intent'] = intent
    
    # Extract shipment info with memory
    weight_match = re.search(r'(\d+)\s*(?:kg|kgs|kilo)', user_msg)
    if weight_match:
        ctx['weight'] = weight_match.group(1)
    
    dest_match = re.search(r'(?:to|for)\s+([A-Za-z]+)', user_msg, re.IGNORECASE)
    if dest_match:
        ctx['destination'] = dest_match.group(1).title()
        ctx['last_discussed'] = f"{ctx.get('weight', 'your')}kg to {ctx['destination']}"
    ...
    context_prefix = ""
    if ctx.get('last_discussed'):
        context_prefix = f"Yes 👍 we discussed {ctx['last_discussed']}. Let's continue. "
    
    # INTENT-BASED RESPONSES
    
    # BALANCED - Not urgent, no risk, no overspend
    if intent == 'balanced':
        response = f"""{context_prefix}Got it 👍 since it's not urgent and you want to avoid risk without overspending, here are your best options:

✈️ Economy Air → safer handling, moderate cost, 7-10 days
🚢 Sea (with proper packing + insurance) → lowest cost, but slower (20-30 days)

I'll help you choose the right balance based on your preference.

Would you prefer faster delivery or maximum cost savings?"""
    
    # NEGOTIATION - Match price
    elif intent == 'negotiate':
        response = """I understand 👍
Let me check if we can match or offer a better option without compromising service.

If I can get you a competitive rate, we can proceed right away.

What's your target price range? I'll see what I can do for you."""
    
    # RESPONSIBILITY
    elif intent == 'responsibility':
        response = """Good question 👍

We ensure responsibility through:
✔️ Clear shipping terms (INCOTERMS)
✔️ Insurance coverage
✔️ End-to-end tracking
✔️ Dedicated support team

In case of any issue, our team actively supports resolution and claim processing.

Would you like me to explain how we handle specific scenarios?"""
    
    # GUARANTEE / Delivery by date
    elif intent == 'guarantee':
        response = f"""It depends on route and shipment details 👍

While exact guarantees depend on carrier schedules, we can check the fastest available option and give you a realistic timeline.

For {ctx.get('weight', 'your')}kg to {ctx.get('destination', 'destination')}, I can check what's possible right now.

Shall I check the fastest delivery option for you?"""
    
    # READY TO BOOK - Strong closing (CONVERSION)
    elif intent == 'ready':
        response = """Perfect 👍 I'll take this forward for you.

Please share your contact number or WhatsApp,
and our team will handle everything from pickup to delivery.

What's the best number to reach you?"""
    
    # COST
    elif intent == 'cost':
        response = f"""{context_prefix}If cost is your priority 👍

🚢 Sea freight is more economical, but takes longer (20-30 days).

For {ctx.get('weight', 'your')}kg of {ctx.get('cargo_type', 'goods')} to {ctx.get('destination', 'destination')}, sea can save you 60-70%.

Would you like me to check the exact sea freight option for you?"""
    
    # URGENT
    elif intent == 'urgent':
        response = f"""{context_prefix}For urgent delivery 👍

✈️ Express air freight is your best option:
• 3-5 days delivery
• Priority handling
• Real-time tracking

For {ctx.get('weight', 'your')}kg to {ctx.get('destination', 'destination')}, I can check flight availability.

Shall I proceed with a quick rate check?"""
...
✔️ Better pricing (volume discounts)
✔️ Dedicated support
✔️ Priority space planning

What's your estimated monthly volume? I'll prepare a custom plan for you."""
...
Need help with a specific shipment? I can connect you with our customs expert."""
    
    # TRUST - High value
    elif intent == 'trust':
        response = """That's a valuable shipment 👍 completely understandable.

We handle such shipments with:
✔️ Trusted airline/shipping partners
✔️ Full tracking visibility
✔️ Insurance coverage
✔️ Experienced handling team

Our focus is not just delivery, but secure delivery without risk.

Would you like me to check the best secure option for you?"""
    
    # ELECTRONICS
    elif intent == 'electronics':
        response = f"""{context_prefix}Electronics need extra care 👍
...
For {ctx.get('weight', 'your')}kg to {ctx.get('destination', 'destination')},

Shall I check the best air freight option for you?"""
...
- Use context from previous messages - never repeat questions
- Suggest air for urgent/valuable items
- Suggest sea for cost-saving/bulk
- Close with a clear next step when user shows interest
...
if __name__ == '__main__':
    print("=" * 60)
    print("Aisea - Revenue-Ready Logistics Consultant (9.5+/10)")
    print("Open: http://localhost:5001")
    print("Fixed: Intent accuracy | Memory | Negotiation | Responsibility | Guarantee | Strong closing")
    print("=" * 60)
