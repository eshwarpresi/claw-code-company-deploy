...
# Intent detection - CRITICAL for dynamic responses
def detect_intent(user_msg, ctx):
    msg_lower = user_msg.lower()
    
    # Dangerous goods
    if any(word in msg_lower for word in ['lithium', 'battery', 'chemical', 'acid', 'flammable', 'explosive']):
    ...
    # Trust / complaint / lost goods
    if any(word in msg_lower for word in ['lost', 'damage', 'trust', 'reliable', 'safe', 'insurance', 'claim', 'forwarder lost']):
        return 'trust'
    ...
    # Ready to proceed / handle everything
...
def chat():
    ...
    print(f"Intent detected: {intent}")  # Debug log
    ...
    # 2. TRUST / COMPLAINT / LOST GOODS
    if intent == 'trust':
        return jsonify({'reply': "I understand 👍 that's a serious concern.\n\nWe handle this with:\n✔️ Insurance coverage\n✔️ Real-time tracking\n✔️ Dedicated support\n\nYour shipment is monitored at every stage. Would you like me to explain our claim process?"})
    
    # 3. NEGOTIATION / MATCH PRICE
    if intent == 'negotiate':
    ...
    # 4. READY TO PROCEED / HANDLE EVERYTHING
    if intent == 'ready':
    ...
    if intent == 'dangerous_goods':
        return jsonify({'reply': "Lithium batteries require special compliance 👍\n\nWe handle:\n✔️ DG documentation\n✔️ Airline approvals\n✔️ Safe packaging\n\nLet me check the best option for this shipment."})
    ...
    # 11. SHIPPING - Extract and respond
    if intent == 'shipping':
        # Extract weight
...
if __name__ == '__main__':
    print("=" * 60)
    print("Aisea - INTENT-DRIVEN 10/10 Chatbot")
    print("Open: http://localhost:5001")
    print("Intents: Tracking | Trust | Negotiate | Ready | Dangerous | Urgent | Budget | Bulk | Customs | Quote | Shipping")
    print("=" * 60)
