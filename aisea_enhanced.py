...
# Rotating closing lines - less spammy
CLOSING_LINES = [
    "Interested in exact rates? I can help with that.",
    "Want me to check current rates for you?",
    "Would you like a firm quote from our team?",
    "Need help with this shipment? Let me know."
]
...
# Hindi responses for localization
HINDI_RESPONSES = {
    "hi": "नमस्ते! मैं Aisea हूँ। कृपया अपना शिपमेंट विवरण साझा करें - वजन और गंतव्य।",
    "rate": "एयर: ₹320-400/kg (3-7 दिन)\nसी: ₹80-120/kg (20-35 दिन)\n\nक्या मैं आपकी मदद कर सकता हूँ?",
    "help": "ज़रूर, मैं आपकी मदद कर सकता हूँ। कृपया अपना शिपमेंट विवरण साझा करें।"
}
...
def chat():
    ...
    # Human handoff - connect to real person
    if 'human' in msg_lower or 'talk to person' in msg_lower or 'real person' in msg_lower or 'customer care' in msg_lower:
        return jsonify({'reply': 'Sure! Let me connect you to our team. Please call +91 90361 01201 or email shivu@pasfreight.com. A representative will assist you immediately.'})
    
    # Hindi support
    hindi_keywords = ['नमस्ते', 'कृपया', 'शिपमेंट', 'वजन', 'भारत']
    if any(word in user_msg for word in hindi_keywords):
        return jsonify({'reply': HINDI_RESPONSES.get('hi', 'नमस्ते! मैं Aisea हूँ। कृपया अपना शिपमेंट विवरण साझा करें - वजन और गंतव्य।')})
    
    # Trust building responses
    if 'delay' in msg_lower or 'guarantee' in msg_lower or 'safe' in msg_lower or 'insurance' in msg_lower:
        return jsonify({'reply': 'We provide real-time tracking, insurance coverage, and dedicated support. Your shipment is fully protected. Need specific details?'})
    
    if session_id not in session_memory:
        session_memory[session_id] = {'conv_index': 0}
    
    conv_index = session_memory[session_id].get('conv_index', 0) % len(CONVERSATIONAL)
    closing_line = random.choice(CLOSING_LINES)
    
    session_memory[session_id]['conv_index'] = conv_index + 1
    
    system_prompt = f"""You are Aisea, a professional logistics assistant for PAS Freight.

RULES:
- Start with conversational opener: "{CONVERSATIONAL[conv_index]}"
- Give estimated rates first
- End with this closing line: "{closing_line}"
- Be natural and friendly
- Never repeat the same line twice

FORMAT:
User: "100kg to Dubai"
You: "Got it 👍 ✈️ Air: ₹320-400/kg (3-5 days)
...
Be conversational, helpful, and natural."""
    ...
    try:
        ...
        return jsonify({'reply': f'Got it 👍\n✈️ Air: ₹320-400/kg (3-5 days)\n🚢 Sea: ₹80-120/kg (20-30 days)\n\n{closing_line}'})

if __name__ == '__main__':
    print("=" * 60)
    print("Aisea - Enhanced 10/10 Professional Chatbot")
    print("Open: http://localhost:5001")
    print("Fixes: Human handoff | Hindi support | Trust building | No repetition")
    print("=" * 60)
